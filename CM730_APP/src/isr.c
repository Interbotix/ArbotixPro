...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define BLINK_COUNT		20//10 //(0.5s * 10 = 5s)
#define BLINK_TIME		521//1042 //(480us * 1042 = 0.5s)
/* 4 cell range
...
/*
	...
	if( gbUSB_Enable == 0 )
	{
		...
		if( bLED_Counter )
		{
			...
			LED_SetState(LED_MANAGE|LED_EDIT|LED_PLAY|LED_RX|LED_TX, OFF);
			LED_SetState(1 << (bLED_Counter % 5) , ON);
		}
		else
			{
			LED_SetState(LED_MANAGE|LED_EDIT|LED_PLAY|LED_RX|LED_TX, OFF);
